<?php

$metadata['https://cirrusgeneral.local.stack-dev.cirrusidentity.com/simplesaml/saml2/idp/metadata.php'] = [
    'metadata-set' => 'saml20-idp-hosted',
    'entityid' => 'https://cirrusgeneral.local.stack-dev.cirrusidentity.com/simplesaml/saml2/idp/metadata.php',
    'SingleSignOnService' => [
        [
            'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
            'Location' => 'https://cirrusgeneral.local.stack-dev.cirrusidentity.com/simplesaml/saml2/idp/SSOService.php',
        ],
    ],
    'SingleLogoutService' => [
        [
            'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
            'Location' => 'https://cirrusgeneral.local.stack-dev.cirrusidentity.com/simplesaml/saml2/idp/SingleLogoutService.php',
        ],
    ],
    'NameIDFormat' => [
        'urn:oasis:names:tc:SAML:2.0:nameid-format:transient',
    ],
    'certData' => '',
];
